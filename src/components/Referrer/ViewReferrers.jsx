        ...
        <section className="w-full mx-auto bg-[#EDEDED] min-h-screen h-auto p-4">
          <div className="p-4 rounded-md bg-white md:p-8 border w-full md:w-[65%] mx-auto shadow-md">
            ...
            {users.map((user, i) => (
              <div
                key={user._id}
                className="w-full mb-4 hover:border hover:border-orange-600"
              >
                  ...
                  <div className="w-full flex p-2 relative ">
                    ...
                    <div className="md:ml-[35px] flex items-center justify-center">
                      ...
                      onClick={()=>navigate(`/profile/${user._id}`)}
                        className="md:w-16 md:h-16 w-[3rem] h-[3rem] object-contain cursor-pointer"
                        src={user.logoURL}
                        alt="logo"
                      />
                    </div>
                    <div className=" max-md:flex max-md:items-center  ml-3 space-y-2 w-full">
                      <h1  onClick={()=>navigate(`/profile/${user._id}`)} className="md:text-xl cursor-pointer text-wrap  font-bold">
                        {" "}
                        <i className="fa-regular fa-building text-orange-500"></i>{" "}
                        {user.companyName}
                      </h1>
                      ...
                      <div className={`md:flex md:flex-col max-md:hidden`}>
                        <h1>
                          {" "}
                          <i className="fa-regular text-orange-500 fa-envelope"></i>{" "}
                          {user.email}
                        </h1>
                        <h1>
                          {" "}
                          <i className="text-orange-500 fa-solid fa-phone"></i>{" "}
                          {user.contact}
                        </h1>
                      </div>
                      <button
                    ...
                    <Link
                      to={user.websiteURL}
                      target="_blank"
                      className="absolute max-md:hidden right-0 top-0 bottom-0 bg-orange-500 w-[30px] h-full flex items-center justify-center text-white font-bold text-xl"
                    >
                  ...
                  {selectedUserId === user._id && (
                    <div className="md:flex md:flex-col mx-auto space-y-2">
                      ...
                      <Link
                        ...
                        className="mt-1"
                      >
                        <i className="fa-solid text-white rounded-md mb-2 text-center fa-globe mx-auto py-2 px-4 bg-orange-500"></i>
                      </Link>
                    </div>
                  )}
                </div>
